...
TDD tests prove `source -> tokens -> DocumentAst -> separate Markdown/frontmatter progress -> metrics -> output` one boundary at a time. Complex stage inputs and expected nested objects live in `test/tdd/fixtures/`, so TDD code only loads data and asserts contracts. BDD scenarios prove what a user sees and the exit status returned by `howdone`; their source-only inputs live in `test/bdd/fixtures/`. `npm test` and `npm run test:bdd` explicitly use the original source runtime, while `npm run test:compiled` explicitly uses the compiled package entry. The same TDD entry files and BDD feature files run in both modes, so the broad matrix protects Unicode, display options, path, error, fixed-sample, and source-to-JavaScript parity behavior.
