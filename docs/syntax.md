# HowDone source and result syntax contract

This document defines the HowDone source and result languages according to the
general conventions of Markdown, the GFM Task List extension, YAML, TOML, and
JSON. Its purpose is to remove ambiguity about lexical composition, parsing,
semantic recognition, rejection, and result mapping. It does not replace or
redefine those underlying format standards.

HowDone is a compiler-shaped pipeline:
...
source text
  ...
  -> progress tree and ProgressReport
  -> terminal or JSON result language
```

This document specifies the source and result languages at those boundaries.
The host driver supplies source text and selects a result serializer; its
invocation syntax is outside this language contract.
...
4. frontmatter-only source and result forms, meaning frontmatter without a
   Markdown body;
5. mixed source and result forms, meaning frontmatter and a Markdown body
   together.

The semantic terms below have distinct meanings:

- A `checkbox` is one recognized leaf checklist item with a boolean state. In
  Markdown, the checkbox marker must be attached to an ordered or unordered
  list item as a GFM task-list marker, for example `- [ ]`, `* [x]`, or
  `1. [X]`; a standalone `[ ]`, `[x]`, or `[X]` in ordinary text is not a
  checkbox. The marker's state is incomplete for `[ ]` and complete for `[x]`
  or `[X]`. In YAML/TOML, the state comes from a boolean value or a leaf
  record's boolean `done` field.
...
| Frontmatter-only | absent  | present     | YAML/TOML progress without Markdown     |
...
each section separate unless an explicit merge is selected to merge the
aggregated frontmatter result with the Markdown result.
...
## 4. Frontmatter-only output

Frontmatter-only means that one or more YAML/TOML frontmatter sections exist and
...
An explicit merge is valid here because the two headers are two source
...
the separate JSON above. If this source had only one header, it would have no
distinct merged result and would retain the single-source mapping.

### 4.3 YAML, YAML, and TOML sections

Several frontmatter sections remain separate source documents even when their
...
Use this frontmatter-only source:
...
`0%`, and `100%`, so the frontmatter-only report has three roots and overall
...
With frontmatter-only input there is no `markdown` member. The report-level
...
The report-level result is therefore `66.67%`. Because the source has two
components, default terminal output expands them into source-labelled sections;
the report-level result remains available in the report and JSON mapping.
...
An explicit merge is a presentation and calculation operation over a source
accepted by the following grammar:
...
`FrontmatterSection+` means one or more sections. The first production is one
body with at least one header; the second is at least two headers without a
body. A single body or a single header is not a mergeable source. Every
frontmatter section counts as one component, and the Markdown body counts as
one component. The operation aggregates frontmatter sections before combining
the aggregated frontmatter side with Markdown when a body exists. It does not
merge repeated YAML sections, repeated TOML sections, or YAML sections into
TOML sections, and it does not alter the individual source-channel results.
...
the merge combine the frontmatter side with the Markdown side.
...
is `F / (F + M)`. An explicit frontmatter share replaces that derived share for
the whole frontmatter side; it does not assign a separate share to each
header. A share affects the combined result only when Markdown and frontmatter
both have checklist roots. A share that cannot affect the selected source
components leaves the ordinary result unchanged.

| Frontmatter order | Frontmatter roots | Markdown roots | Default frontmatter share |                                                        Example report-level result |
|-------------------|------------------:|---------------:|--------------------------:|-----------------------------------------------------------------------------------:|
...
second YAML section, and `100%` for Markdown. When an explicit merge is
selected, the merged result has three roots and `2 / 3 = 66.67%`. With an
explicit share of `0.5`, the frontmatter side is weighted as one side:
`0.5 * 0.5 + 0.5 * 1 = 0.75`, so the result becomes `75%`.

The explicitly merged tree keeps both YAML roots before the Markdown root:
...
The corresponding explicitly merged details are:
...
The explicitly merged JSON is still one JSON object document. It keeps two
independent `frontmatter` entries, followed by `markdown`; `progress` contains
all three roots:
...
YAML and Markdown roots are incomplete. When an explicit merge is selected, the
merged result is `1 / 3 = 33.33%`; its derived frontmatter share is still `2 / 3`
because there are two frontmatter roots.
...
Its explicitly merged details are:
...
For the source in section 5.1, the explicitly merged tree is:
...
The explicitly merged result has `Root nodes: 2`, `Explicit checkboxes: 4`,
...
An explicit share of `0.5` produces the same `66.67%` result for this source and
changes only the merge calculation. It does not change `markdown`,
`frontmatter`, or their individual trees.

If an explicit share has no frontmatter checklist roots or no Markdown
checklist roots, it cannot affect the combined result. The merge itself still
follows the source-component rule and leaves the individual source results
unchanged.
...
The expanded tree and details examples below use complete labels; JSON labels
...
complete, and the Markdown root is incomplete, so the report-level result is
`50%`. Because the source has three components, default terminal output
expands each component in source order.
...
contributes one root. The explicitly merged result is therefore `50%`. An
explicit share of `0.5` replaces the derived share and produces `33.33%`.
...
- an explicit merge request selects the second operation.
...
The result mapping is independent of the host serializer's display defaults.

JSON output is a data document with raw numeric fields. Text formatting and
label presentation do not alter those fields. JSON labels are complete by
default; a display-specific serialization may request grapheme-safe label
truncation without changing the Core progress data. An explicit frontmatter
share affects only the merged report calculation when both source sides have
checklist roots; otherwise the ordinary result is unchanged.
...
JSON labels are complete by default. A serializer may request grapheme-safe
truncation, while a no-truncation presentation keeps labels complete.
Truncation changes presentation only; it does not mutate core progress data.
The JSON serializer writes one JSON object document. It is not a JSON string
containing escaped object text, so tools such as `jq` can parse and reformat it
directly.
